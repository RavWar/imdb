module Imdb
  ...
  class Base
    ...
    def initialize(imdb_id, imdb_type = nil, title = nil, also_known_as = [])
      @id    = imdb_id
      @type  = imdb_type
      @url   = "https://imdb.com/title/tt#{imdb_id}/reference"
      @title = title
    ...
    # Returns an array of countries as iso-strings.
    def iso_countries
      document.search("h5[text()='Country:'] ~ div a[@href*='/country/']/@href").map { |href| href.value.match(/\/country\/(.{2})/)[1].upcase } rescue []
    end
    ...
    def title(force_refresh = false)
      if @title && !force_refresh
        ...
        @title = CGI.unescapeHTML(document.at(".combined h1").inner_html.split('<span').first.strip.gsub(/"/, '')) rescue nil
      end rescue nil
    end
    ...
    def self.find_by_id(imdb_id, page = :reference)
      Imdb.fetch("https://imdb.com/title/tt#{imdb_id}/#{page}")
    end
